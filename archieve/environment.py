...
class BaseEnv:
    ...
    def reset(self):
        print("[BaseEnv.reset()] START", flush=True)
        ...
        print("[BaseEnv.reset()] Creating scene...", flush=True)
        scene = self._create_scene()
        xml_string = scene.to_xml_string()
        assets = scene.get_assets()

        print("[BaseEnv.reset()] Building model...", flush=True)
        ...
        if self._render_mode == "gui":
            ...
            print("[BaseEnv.reset()] Creating offscreen renderer...", flush=True)
            self.viewer = mujoco.Renderer(self.model, 128, 128)
            print("[BaseEnv.reset()] Offscreen renderer created!", flush=True)
        ...
        self._t = 0
        print("[BaseEnv.reset()] END", flush=True)
        print(f"self high level state {self.high_level_state}", flush=True)
